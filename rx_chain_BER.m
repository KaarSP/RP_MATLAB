...
figure;
plot(autocorrResult)
title('Auto correlation result - No jamming')
figure;
...
receivedBits = real(I_out) > 0;       % BPSK decision rule (threshold at 0)
...
figure
plot(referenceBits(1:1000),'b');
hold on;
plot(receivedBits(1:1000),'r');
title('no jamming')
...
figure;
plot(autocorrResult_gauss,'r')
title('Auto correlation result - No jamming')
figure;
plot(repeatedSequence_gauss,'r.')
title('Sequence after auto-correlation')
...
plot(referenceBits(1:1000),'b');
hold on;
plot(receivedBits_gauss(1:1000),'r');
axis([1 1000 -0.5 1.5])
title('gaussian')